          ...
          <nav className="nav" aria-label="Main navigation">
            {navItems.map((item) => {
              ...
              return item.children ? (
                <div
                  className={`nav-dropdown ${openDropdown === item.label ? "nav-dropdown-open" : ""} ${active ? "nav-active" : ""}`}
                  key={item.label}
                ...
                <Link className={active ? "nav-active-link" : ""} aria-current={active ? "page" : undefined} key={item.href} href={item.href} onClick={closeMenus}>{item.label}</Link>
              );
            })}
          </nav>
      ...
      <aside className="mobile-menu-panel" aria-label="Mobile menu">
        ...
        <nav className="mobile-panel-nav">
          ...
          {navItems.map((item) => {
            ...
            return (
              <div className={`mobile-nav-group ${active ? "mobile-nav-active" : ""}`} key={item.label}>
                {item.children ? (
                  <button className="mobile-nav-parent mobile-nav-toggle" type="button" aria-current={active ? "page" : undefined} aria-expanded={isOpen} onClick={() => setOpenMobileDropdown(isOpen ? null : item.label)}>
                ...
                {item.children && isOpen ? (
                  <div className="mobile-nav-children">
                    {item.children.map((child) => {
                      const childActive = pathname === child.href || pathname.startsWith(`${child.href}/`);
                      return (
                        <Link className={`mobile-nav-child ${childActive ? "mobile-nav-child-active" : ""}`} aria-current={childActive ? "page" : undefined} key={child.href} href={child.href} onClick={closeMenus}>
