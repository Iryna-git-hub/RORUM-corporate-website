...
export const cateringContract: PageContentContract = {
  ...
  entries: [
    ...
    // ---- gallery + "suitable for" --------------------------------------
    {
      ...
      frontendSelector: "#catering-gallery gallery items (photos render as <img>, videos as a real <video controls>)", expectedBehavior: "Adding/removing/reordering media (photo or video) changes the gallery accordingly, in the exact order stored.",
      mutationStrategy: "reorder", editorVisibility: "visible", classification: "connected",
      notes:
        'FIXED this pass — previously video items (kind="video") were filtered out before rendering (HorizontalGallery was images-only), so a manager-added video silently never appeared on the live site despite Studio accepting it. Now: an uploaded videoFile resolves via sanity/lib/image.ts\'s urlForFile() (preferred over videoUrl when both are set); a direct videoUrl (e.g. ending in .mp4/.webm, matched against the parsed pathname case-insensitively) renders via <video src>; a YouTube/Vimeo watch-page link (or any other non-file webpage URL) is rejected by mediaItem.ts\'s own Studio validation and, as defense in depth, dropped (not rendered) by resolveGalleryItems() if it ever reaches the frontend — provider embeds (iframe) are NOT implemented, matching the field\'s own "direct link to a video file" description exactly. 59 real uploaded photos live today (verified), zero video items live today (verified). Video does NOT use or require a separate poster image — a poster can crop/frame the subject differently than the video itself, so mediaItem.ts\'s posterImage field is hidden and unvalidated site-wide (superseded an earlier, since-reverted poster-required implementation); the gallery instead shows the video\'s own frame, inside a fixed 16:9 card via CSS object-fit:contain (never cropped/stretched), with a neutral loading surface until the first frame decodes and a neutral localized "Video unavailable" state (lib/uiText.ts, EN/DA/UK) on runtime playback failure — never an image in either state. Alt text (EN/DA/UK, required) is the video\'s only accessible-name source, used as its aria-label. The photo lightbox stays photo-only; a video plays inline in the main track with native controls, no autoplay. An intentionally emptied gallery (media: []) renders empty, not the static fallback set — see lib/sanityGallery.ts and components/HorizontalGallery.tsx.',
    },
    {
