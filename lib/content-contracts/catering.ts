...
export const cateringContract: PageContentContract = {
  ...
  entries: [
    ...
    // ---- gallery + "suitable for" --------------------------------------
    {
      ...
      fieldPurpose: "Ordered gallery photos AND videos, in the manager's own array order", fieldType: "repeatable", required: true, languages: [],
      querySource: CATERING_QUERY, mapper: "getData() galleryItems, via resolveGalleryItems()", component: "components/HorizontalGallery.tsx",
      frontendSelector: "#catering-gallery gallery items (photo/video cards in the main track) + the shared Lightbox they all open into", expectedBehavior: "Adding/removing/reordering media (photo or video) changes the gallery accordingly, in the exact order stored; clicking any item (photo or video) opens the Lightbox at that item's exact position.",
      mutationStrategy: "reorder", editorVisibility: "visible", classification: "connected",
      notes:
        'FIXED this pass — previously video items (kind="video") were filtered out before rendering (HorizontalGallery was images-only), so a manager-added video silently never appeared on the live site despite Studio accepting it. Now: an uploaded videoFile resolves via sanity/lib/image.ts\'s urlForFile() (preferred over videoUrl when both are set); a direct videoUrl (e.g. ending in .mp4/.webm, matched against the parsed pathname case-insensitively) renders via <video src>; a YouTube/Vimeo watch-page link (or any other non-file webpage URL) is rejected by mediaItem.ts\'s own Studio validation and, as defense in depth, dropped (not rendered) by resolveGalleryItems() if it ever reaches the frontend — provider embeds (iframe) are NOT implemented, matching the field\'s own "direct link to a video file" description exactly; mediaItem.ts also requires a video to have AT LEAST ONE usable source (uploaded file or valid URL) before Publish, closing the gap where neither was ever set. 59 real uploaded photos live today (verified), zero video items live today (verified). Video does NOT use or require a separate poster image — a poster can crop/frame the subject differently than the video itself, so mediaItem.ts\'s posterImage field is hidden and unvalidated site-wide (superseded an earlier, since-reverted poster-required implementation); every video shows its own frame instead, via CSS object-fit:contain (never cropped/stretched), with a neutral loading surface until the first frame decodes and a neutral localized "Video unavailable" state (lib/uiText.ts, EN/DA/UK) on runtime playback failure — never an image in either state. Alt text (EN/DA/UK, required) is the video\'s only accessible-name source. Photos AND videos are now equal members of ONE ordered Lightbox (superseded an earlier, since-reverted photo-only Lightbox design): the main track shows a small preview card per item — a photo opens directly, a video preview has a separate "Open video" button (the small card itself has no native controls, avoiding a nested-interactive-element conflict with the track\'s own drag/swipe); inside the Lightbox, only the ACTIVE slide is a real interactive <video controls> — neighboring preview slides show a silent frame. Navigating away from, or closing, a playing Lightbox video always pauses it first. An intentionally emptied gallery (media: []) renders empty, not the static fallback set — see lib/sanityGallery.ts and components/HorizontalGallery.tsx.',
    },
    {
