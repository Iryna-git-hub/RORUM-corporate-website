...
4. **Social links fixed** (`lib/sanityContact.ts`'s `resolveSocialLinks`, `socialLink.ts`): brand color is now derived purely from platform, not read from the manager-typed `brandColor` field — this silently fixes the live LinkedIn entry's wrong black color without touching its stored data (superseded — see Part 22: RORUM does not want a LinkedIn profile on Contact/Header/Footer at all, so this was fixing data that turned out to be unwanted legacy content, not a genuine profile); `brandColor` is hidden in Studio (kept, not deleted) with an updated description; accessible label is now required en/da/uk (`requireAllLanguages()`), correctly surfacing the live LinkedIn entry's missing da/uk translation as a visible Studio error (not silently invented — left for the manager to fill in); a duplicate-platform validation error was added; the missing-vs-empty fallback bug was fixed (`doc?.links?.length ? ... : fallback` -> `doc == null ? fallback : doc.links`, so an intentionally-emptied list renders empty, never resurrects the hardcoded starter list).
...
**Correction (not "pure backend")**: Part 21's own work includes real frontend/layout changes, not just Studio schema/backend plumbing — `ContactForm.tsx`'s field rendering was refactored from a fixed 2-column Phone+Email row to a single-column stack (a genuine layout decision, see the visual-baseline note above), `FAQInlinePrompt`/`PrivacyConsent` gained new conditional-rendering paths, and `app/[locale]/(site)/contact/page.tsx`'s JSX was restructured to map over a manager-configured detail order. Described precisely here to avoid understating the change's scope.

## Part 22 — Verification follow-up: Social Links business correction, Event Decoration seo residue resolved, corrected reporting

A follow-up review of Part 21/the verification report caught 3 things that needed correcting before this work is ready to commit — all resolved, none published/committed without authorization.

### 1. Social Links: LinkedIn is unwanted legacy content, not a profile that needed repairing

Part 21 point 4 (above) treated a live LinkedIn entry's wrong brand color and malformed href as data-quality bugs to fix. **That framing was wrong.** RORUM does not have a LinkedIn profile that should appear via the shared `socialLinks` singleton (Contact/Header/Footer) — the earlier href/label repair (already applied to *published* `socialLinks` before this correction) fixed data that should never have been published as a "real" profile in the first place. Corrected now:

- **`drafts.socialLinks`'s absence of LinkedIn is intentional**, not a gap. A full semantic diff (by platform, comparing every field) confirms Instagram and Facebook are **byte-identical** between `socialLinks` (published) and `drafts.socialLinks` — same `_key`, `href`, `brandColor`, and EN/DA/UK label on both. LinkedIn is the **only** difference: present in published, absent in draft. Publishing the draft as-is will not alter Instagram or Facebook in any way, and will remove LinkedIn as its only effect.
- **Not merged, not published by this session.** `drafts.socialLinks` was never written to (this session's earlier href/label repair explicitly detected and skipped it, logged as "no link with _key ... — skipped" — confirmed correct in hindsight, not just accidentally so). No publish action was taken on `socialLinks` — per the task's own instruction, that is the manager's own action to take in Studio once they've reviewed this.
- **Schema narrowed to match the business decision**: `socialLink.ts`'s "Platform" dropdown (used *only* by the `socialLinks` singleton — confirmed via a full-codebase search that Event Share's own share-platform list is a separate, hardcoded schema in `event.ts` that does not reuse this type) now offers exactly Instagram and Facebook, nothing else. **Correction of an assumption made mid-task**: this was initially assumed to be picker-only (not retroactively invalidating an already-stored value) — re-verified live and that assumption was wrong. Sanity's `options.list` on a plain string field *does* retroactively flag an out-of-list stored value ("Value \"linkedin\" did not match any allowed values"), which now shows on the *published* `socialLinks` document (still holding the stray LinkedIn entry). This is expected and correct, not a regression to route around: Studio's Publish button is governed by the *draft's* own validation — `drafts.socialLinks` has neither this error nor a LinkedIn entry at all — so the manager can still publish the clean draft normally, and the published document's transient error disappears the instant they do (publishing replaces it with the draft's content entirely). Confirmed via the official validator: `drafts.socialLinks` = 0 errors; `socialLinks` (published) = 2 markers, both the same "linkedin did not match any allowed values" on the one stray entry.
- **Event Share LinkedIn is explicitly untouched and still fully supported** — `components/EventShare.tsx`, `sanity/schemaTypes/documents/event.ts`'s `SHARE_ACTION_TYPES`, and `components/SocialIcon.tsx`'s shared icon renderer (still needed by Event Share) were not modified. New regression tests (`components/EventShare.test.tsx`) prove a `linkedin`-type share action still renders a real, working LinkedIn share link, independent of the `socialLinks` singleton's own platform list.
- Backed up (`scripts/backup-contact-docs.ts`, label `pre-social-links-business-correction`) before this review; no further mutation was made to `socialLinks`/`drafts.socialLinks` data itself in this pass — only the schema's selectable-platform list changed.

### 2. Event Decoration: the actual, precise validation state (corrects an earlier reporting error)

The prior verification report's closing summary line ("Event Decoration: 0 errors") was **inaccurate** — it conflated the *published* document's error count with the combined picture. The correct, precise state at the time was:

| Document | `_rev` | `_updatedAt` | Validation errors |
|---|---|---|---|
| `page-event-decoration` | `EfB2kIrJQHDYmeHeg0zsly` | 2026-08-24T09:46:40Z | 0 |
| `drafts.page-event-decoration` | `ad4adee6-e21d-4ef7-89b4-3d4b48722727` | 2026-08-24T11:04:07Z | 1 (`seo.title[_key="01705d3487af"].language` — "Required") |

Command used (this Sanity CLI version validates the whole dataset in one pass — there is no per-document invocation — so results are filtered from that output by exact `documentId`): `SANITY_AUTH_TOKEN=<token> npx sanity documents validate --yes --level error --format json`.

**Root cause, re-confirmed precisely**: the one blocking entry has *only* `_key`/`_type` — no `language`, no `value`, no other property — and was the sole entry in `seo.title`; the rest of `seo` (`description`, `ogImage`) was entirely empty on this draft too. It could never have rendered: every SEO consumer reads via `pickLocalized()`, which matches by `entry.language === locale` — an entry with no `language` at all can never match, so it was inert to every reader before the fix, not just after.

**Cleanup applied** (`scripts/repair-event-decoration-seo-residue.ts`, revision-guarded, `--apply`-gated, authorized before running):
- Backed up `page-event-decoration` + draft (`scripts/backup-event-decoration-docs.ts`, label `pre-seo-residue-cleanup`).
- Snapshotted the draft's gallery `media` array (14 items, exact `_key` order including the manager's own reorder — `img4` before `img3`, `img7` before `img6`) *before* patching.
- Patch targeted exactly `seo.title[_key=="01705d3487af"]` via `unset()` — a single array member by `_key`, nothing else.
- **Verified after applying**: re-dry-run reports "already clean, nothing to do" (idempotent); official validation reports 0 errors on *both* `page-event-decoration` and `drafts.page-event-decoration`; the gallery `media` array re-fetched afterward is **identical** (same 14 keys, same order) — the manager's pending video-removal and photo reorder are fully preserved.
- `drafts.page-event-decoration` was never published or discarded by this session. Publish is now available in Studio for the manager to use once they choose to.

### 3. Contact configuration authority — map independence, confirmed by code path (not just claimed)

Re-verified against the final code: the embedded Google Map (`app/[locale]/(site)/contact/page.tsx`) reads `data.contactDetails.mapQueryAddress` — sourced from `resolveContactDetails(contactInfoDoc)`, a resolver entirely separate from `resolveContactDetailOrder()` — and renders in its own unconditional `<section>`, never inside the `data.contactDetailOrder.map(...)` loop that drives the Address/Phone/Email detail rows. Hiding "Address" from the manager-controlled detail-order list structurally cannot hide the map; they share a data source (`contactInfo`) but not a rendering or visibility path.

### 4. Live real-browser verification, exact revert

Task 4's browser proof (Add City / remove Email / reorder / FormData contents / no network request on submit / no PII logged / EN-DA-UK resolution) required a temporary live mutation to `page-contact` (published) — documented in full here since the temporary script and test file were deleted after use, per this project's established "disposable diagnostic" convention:
- Backed up first (`scripts/backup-contact-docs.ts`, label `pre-task4-live-verification`).
- Snapshotted the form section's exact 6-item array before mutating.
- Applied a single test mutation: removed `field-email`, added a new `field-city` item, reordered to `[message, city, name, phone, successMessage, submitLabel]`.
- Ran 7 real-browser Playwright tests against this live state — all passed (see the verification report for the full list).
- **Reverted** immediately via the saved snapshot, then **verified byte-for-byte** that the reverted array matched the pre-mutation backup exactly (`JSON.stringify` equality check, not just visual inspection).
- Re-confirmed official validation stayed clean afterward.
- No trace of this temporary mutation remains in the dataset or the working tree.

### 5. Known pre-existing gap: no dedicated automated tests for Volunteer/CV forms

`components/VolunteerApplicationForm.tsx` and `components/CvUploadModal.tsx` have no dedicated unit or component test files in this project, and `tests/interactions.spec.ts` does not exercise them either (only Contact's form is covered there). This is a **pre-existing gap**, not something this session's Contact work introduced or hid — verified instead by code-isolation reasoning: neither file was touched this session, and every new Contact-specific schema predicate (`isPageContact`, the 6 new `ITEM_ROLE_RULES` rows) is scoped by document id/`sectionKey` in a way that structurally cannot match Volunteer's own document or sections. Flagged here as a real, un-closed coverage gap for a future task, not swept under a passing test run.

### 6. Working-tree incident and cleanup
...
## Part 25 — Events Listing: manager-friendly filter groups (commit `9ba36e5`, baseline `5a9c50d`)

Authorized to run autonomously immediately after Part 24's SEO commit, following that task's own verbatim prompt. Nothing here rewrites or reverts a previously-pushed commit.

### 1. Read-only audit — exact findings

`page-events` has 3 sections: `hero` (title only — this IS the public H1, confirmed: `app/[locale]/(site)/events/page.tsx` reads `heroSection?.title`, never `filtersSection`'s own title/label), `filters` (14 items pre-migration, all read via `getItem(filtersSection, key)?.title` only — never `.text`/`.href`/`.label`/`.value`/`.icon`/`.image`), `closingCta` (label/title/text/1 action/2 items/1 settings row). Two provable dead-data findings:
- `filters`'s own section-level `label`/`title` are never read anywhere (only per-ITEM `.title` is) — hiding them was safe.
- `closingCta.settings` (`variant: "host"`) is stored but the frontend hardcodes `variant="host"` in JSX directly — but this needed **no new rule**: sectionKind `"cta"`'s own existing `FIELD_VISIBILITY` already omits `settings` for every closingCta on every page, Home/About included.
- **The most significant finding**: `closingCta.items` stores `faqQuestion`/`faqLabel` (matching the shared "Closing CTA FAQ prompt row (Home + About)" role Home/About genuinely use) — but `events/page.tsx` builds its "Have questions?" prompt entirely from `formMessages.faqQuestion`/`.faqLabel` (`data.faqQuestion = messages.faqQuestion`), **never** from this section's own items. These 2 rows are live, stored, but 100% inert. Presenting them as an editable override (the way Contact's own analogous FAQ-prompt-override genuinely works) would have been exactly the "fake editable field the frontend ignores" the task explicitly warned against — resolved per §7 below.
- Confirmed via `lib/eventLanguage.ts`: the Language filter's 3 option display names ("English"/"Danish"/"Ukrainian") were hardcoded in code, with no corresponding Sanity rows at all.
- **A load-bearing correction to the task's own stated assumption**: the task's Section 6 assumed the Language filter's underlying values are the locale codes `da`/`en`/`uk`. Live audit disproved this — `components/EventsClientPage.tsx`'s `languageOptions` are the distinct `event.language` values actually present among loaded events ("English"/"Danish"/"Ukrainian", matching `event.language`'s own `options.list`), a completely different concept from the site's own display locale. The 3 new `languageXxLabel` rows were implemented against this real value domain (each row supplies the manager-editable display name for one `event.language` value, in the site's current display locale) rather than the assumed-but-nonexistent locale-code scheme — reported here rather than silently forcing a scheme that doesn't match the real filtering logic.

### 2. Studio field visibility

`sanity/schemaTypes/objects/pageSection.ts`: new `isPageEvents()`; `hero`/`filters`/`closingCta` sectionKey/sectionKind hidden unconditionally (same pattern as Contact's fixed hero/form sections); `filters`'s `label`/`title` force-hidden (page-events only — defensive regression tests prove every other page keeps them visible). `sanity/schemaTypes/objects/contentItem.ts`: the pre-existing (undocumented-scope) "Events filter/empty-state label" role is now `documentIds: ["page-events"]`-scoped, `requiredFields: ["title"]` (blank labels would produce unusable controls), and its itemKeyPattern extended with the 3 new `languageXxLabel` keys — 17 keys total, all `visible: ["title"]` only. `ITEM_KEY_PREVIEW_LABELS` gained all 17 entries for readable fallback previews.

### 3. Filter groups — real reorder, not cosmetic

New `sanity/components/EventsFiltersInput.tsx` (chained into the existing `pageSection.items` chain after `ContactFormItemsInput`) renders 5 groups — **Date / Language / Price / Availability / Filter messages** — each option row using the existing default single-item editor (title always shows EN/DA/UK immediately, via `RoleAwareAllLanguagesInput`'s role-prefix check extended to also cover `"Events "` roles). No add/duplicate/copy/remove anywhere (`disableActions`, not CSS) — these 17 rows are a fixed, non-extensible semantic set. Move up/down is scoped to stay strictly inside each option's own group (never crossing into another group or touching a heading row), using the same key-addressed `unset`+`insert` `PatchEvent` primitive `ContactDetailsOrderInput.tsx` established, never a full-array `set(next)`. A row missing from storage shows an explicit "Missing" card (with no Move controls, since there's nothing to move) rather than being silently omitted or fabricated.

**The reorder is real, not cosmetic** — the task's own Section 9 explicitly warned against building a reorder UI the frontend ignores, and the *first* implementation draft would have done exactly that (`components/EventFilters.tsx`'s Date/Price/Availability option arrays were hardcoded literals, completely independent of stored array order). Fixed by extracting a new pure resolver, `lib/eventFilters.ts`'s `resolveOrderedFilterOptions()`, which reads the manager's own stored item order (filtered to that group's known keys, any genuinely-missing known key appended at the end rather than dropped) and returns `{value, label}` pairs — `value` is always the fixed stable string the URL/filtering logic already expects, completely unaffected by reordering or relabeling. Threaded through `events/page.tsx` → `EventsClientPage.tsx` → `components/EventFilters.tsx` (which now accepts optional `dateOptionOrder`/`priceOptionOrder`/`availabilityOptionOrder`, falling back to its own previous hardcoded arrays when absent — e.g. the `!isSanityConfigured` static path). Verified live: `tests/cms-events-contract.spec.ts`'s new tests and `lib/eventFilters.test.ts`'s 17 unit tests both prove stored order — not code order — determines what's rendered.

### 4. Language option labels — real value domain, not the assumed one

`lib/eventFilters.ts`'s `resolveEventLanguageLabels()` reads the 3 new CMS rows (falling back to `lib/eventLanguage.ts`'s existing hardcoded table when unset), keyed by the real `event.language` string values. `components/EventFilters.tsx` gained an optional `languageLabels` prop, preferred over the hardcoded `getEventLanguageLabel()` lookup — `event.language`'s own stored values and `selectedLanguage`'s URL param are completely untouched by any of this; only the *displayed text* changes. `lib/eventLanguage.ts` itself, and its use on the Event Detail page (out of this task's scope — "do not modify individual Event document editing"), is unchanged.

### 5. Closing CTA — read-only explanation instead of a fake override

New `sanity/components/EventsClosingCtaItemsInput.tsx` (chained into the same `items` chain, page-events' closingCta section only) replaces the native items editor for `faqQuestion`/`faqLabel` with a read-only card explaining the live prompt comes from Shared form messages, plus a working `IntentLink` to that singleton — matching the task's own explicit "show a clear read-only explanation rather than a fake editable field" instruction for exactly this class of gap. The stored `faqQuestion`/`faqLabel` items themselves are untouched (never deleted or unset) — only their Studio presentation changed. Every other closingCta field (label/title/text/actions — all genuinely read by the frontend) renders through the untouched default form.

### 6. Data migration — backup, dry-run, apply, idempotency proof

Order followed exactly: `npm run sanity:backup-events-listing` → `sanity:migrate-events-filter-labels:dry-run` (reviewed) → `sanity:migrate-events-filter-labels` (apply) → re-run dry-run (idempotency) → official validation.
...
New: `lib/eventFilters.ts` (+`.test.ts`), `sanity/components/EventsFiltersInput.tsx` (+`.test.tsx`), `EventsClosingCtaItemsInput.tsx` (+`.test.tsx`), `scripts/backup-events-listing-docs.ts`, `migrate-events-filter-labels.ts`.

Modified: `app/[locale]/(site)/events/page.tsx` (now uses `lib/eventFilters.ts`'s resolvers instead of an inline `filterField()` loop; the redundant `fallbackFilters` local constant — byte-identical to `EventFilters.tsx`'s own `defaultEventFilterLabels` — was removed in favor of the one shared source), `components/EventFilters.tsx` (+`languageLabels`/`dateOptionOrder`/`priceOptionOrder`/`availabilityOptionOrder` props), `components/EventsClientPage.tsx` (threads the 4 new props through), `sanity/components/ContactFormItemsInput.tsx` (chains to `EventsFiltersInput`), `sanity/components/RoleAwareAllLanguagesInput.tsx` (+`.test.tsx`) (generalized `isContactRole` → `isAlwaysAllLanguagesRole`, now also matching `"Events "`-prefixed roles), `sanity/schemaTypes/objects/contentItem.ts`, `pageSection.ts`, `package.json` (4 new npm scripts), `tests/cms-events-contract.spec.ts`, `tests/sanity-schema-visibility.spec.ts`.

### 8. Test results

`npm run typecheck`: clean throughout. `npm run lint`: 0 errors, only the same pre-existing warnings from before this Part. `npm run build`: succeeded. `npx vitest run`: **450/450 passed** (33 files — 45 new tests this Part: 17 in `lib/eventFilters.test.ts`, 11 in `EventsFiltersInput.test.tsx`, 5 in `EventsClosingCtaItemsInput.test.tsx`, 2 new in `RoleAwareAllLanguagesInput.test.tsx`). `npx playwright test tests/sanity-schema-visibility.spec.ts`: **272/272 passed** (13 new tests: `isPageEvents` id-recognition, the 3 fixed sections' hidden fields with cross-document regression proofs, and the "Events filter/empty-state label" role's document-scoping/required-title proofs). `npx playwright test tests/cms-events-contract.spec.ts`: **28/28 passed**, including live proof that the Language filter dropdown shows the CMS-sourced label, empty-state text matches Sanity, closing CTA renders from `page-events` (not Home/About's), and — the key regression proof — that the "Have questions?" prompt genuinely uses `formMessages`, not `page-events`'s own (dead) `faqQuestion`/`faqLabel` items. Broader regression sweep (`interactions.spec.ts`, `locale.spec.ts`, `cms-home-contract.spec.ts`, `cms-about-contract.spec.ts`, `cms-catering-contract.spec.ts`, `cms-event-decoration-contract.spec.ts`, `seo.spec.ts`, `visual.spec.ts`): **all green** except the 2 pre-existing failure classes already documented in Part 24 §17 (Catering's published-vs-draft SEO gap, awaiting manual publish; Event Decoration's unrelated gallery photo/video count mismatch) — no new failures, no visual baseline regenerated.

### 9. Manual Studio checklist

1. Open `page-events` → Filters — confirm Key/Section type are hidden, and the 5 groups (Date/Language/Price/Availability/Filter messages) render with no flat mixed Items list.
2. Edit one option's label (e.g. "Soonest first") and confirm EN/DA/UK are all visible immediately.
...
## Part 26 — Canonical domain correction and an honest Studio SEO preview (baseline `0f15109`)

### 1. Root cause

Two independent, confirmed defects, both pre-existing (not introduced by any prior Part in this report):

1. **Domain authority was inconsistent.** `lib/siteConfig.ts`'s `companyDetails.website` correctly held `"ro-rum.dk"`, but `lib/data.ts`'s `siteUrl`, `lib/siteConfig.ts`'s `companyDetails.siteUrl`, both root layouts' `metadataBase`, `sanity/components/SeoObjectInput.tsx`'s own `SITE_URL` constant, every SEO-related test file, and the live `siteSettings.siteUrl`/`drafts.siteSettings.siteUrl` Sanity fields all held `"https://rorum.dk"` (no hyphen) — the wrong domain. On top of the wrong static value, `lib/siteSettings.ts`'s `getSeoSiteDefaults()` additionally let the manager-editable `siteSettings.siteUrl` field override even that at runtime — meaning canonical/hreflang/OG/sitemap URLs were controlled by ordinary, accidentally-stale-able Sanity content rather than a single infrastructure fact.
2. **The Studio SEO preview didn't show the effective fallback.** `SeoObjectInput.tsx` showed a hardcoded `"(not set for this language...)"` placeholder and a `"Title: using fallback"` badge for an empty `seo.title`/`.description` — never the actual text a visitor or search engine would receive. For an Event specifically, the public route resolves an empty `seo.title` to `${event.title} | RORUM` and an empty `seo.description` to `event.longDescription` — Studio showed neither, so a manager reviewing an Event with no SEO override had no way to see what was actually being published.

### 2. Old/new domain authority

- **Wrong (removed everywhere):** `https://rorum.dk`.
- **Correct (now the one source):** `https://ro-rum.dk`, exported as `PRODUCTION_ORIGIN` from new `shared/siteIdentity.ts` — a dependency-free module (no React/Next.js/Sanity/browser/server-only code) importable by Next server code, Sanity Studio client code, scripts, and tests alike. Also exports `normalizeOrigin()` (trailing-slash-safe), `buildUrl()` (origin+path join, never a double slash), `isProductionOrigin()`, and `resolveProductionOrigin()` (a defensive resolver for a future, explicitly-named production env override — none exists in this codebase today, and none was added; every real call site resolves the bare `PRODUCTION_ORIGIN`).
- **`siteSettings.siteUrl` is no longer a runtime authority at all.** `lib/siteSettings.ts`'s `getSeoSiteDefaults()` always returns `PRODUCTION_ORIGIN` for `siteUrl`, regardless of what's stored in Sanity — canonical/hreflang/sitemap/robots/OG/JSON-LD URLs can no longer be changed by editing that field. The Studio field itself is kept (not deleted, preserving stored data) but is now `readOnly: true` with an `initialValue` of `"https://ro-rum.dk"` and a description explaining it's fixed infrastructure — `siteSettings.website` (`"ro-rum.dk"`, already correct) remains the one manager-editable display/legal-text field, unaffected. `companyDetails.siteUrl` (`lib/siteConfig.ts`, confirmed unused by any other code) and `lib/data.ts`'s `siteUrl` export both now read `PRODUCTION_ORIGIN` too, so there is exactly one place the value is ever hardcoded.
- Business email addresses (`hello@rorum.dk`, found only in `scripts/import-translations.ts`/`import-pages.ts`'s legal-page body copy) were **not** touched — confirmed to be real contact addresses, not the website domain, per the task's own explicit instruction.

### 3. Exact SEO resolution precedence (shared contract)

New `shared/seoResolution.ts` (dependency-free): `resolveSeoField(tiers)` returns the first tier (in caller-given order) whose trimmed value is non-empty, tagged with its `source` (`"documentOverride" | "documentContent" | "pageDefault" | "siteDefault" | "emergencyDefault"`); `resolveSeo()` bundles `{title, description, canonicalUrl}` in one call. Both `lib/seo.ts` (the public metadata generator) and `sanity/components/SeoObjectInput.tsx` (the Studio preview) call the exact same `resolveSeoField` — each gathers its own tier *values* (unavoidably environment-specific: Next reads already-fetched page/event data, Studio reads `useFormValue` plus a live client-side `siteSettings` fetch), but the precedence algorithm itself is shared code, not reimplemented twice.

- **Static pages** (`page`/`legalPage`): `documentOverride` (this document's own `seo.title`/`.description`) → `pageDefault` (the page's own hardcoded English fallback — unchanged, still lives in each `page.tsx`'s own `fallback` constant; also duplicated read-only into new `shared/pageSeoDefaults.ts` solely so Studio can display the same exact text) → `siteDefault` (`siteSettings.defaultSeo`) → `emergencyDefault` (`shared/seoResolution.ts`'s `EMERGENCY_SEO_TITLE`/`EMERGENCY_SEO_DESCRIPTION`, "RORUM" / "RORUM — events, community and creative space in Copenhagen.").
- **Event Detail**: `documentOverride` (`event.seo.title`/`.description`) → `documentContent` (`${event.title} | RORUM` / `event.longDescription` — generated live from the document's own other fields) → `siteDefault` → `emergencyDefault`. `app/[locale]/(site)/events/[slug]/page.tsx`'s `generateMetadata()` was corrected to stop falling back to a page-level hardcoded description string before `documentContent` exhausted — it previously short-circuited past `siteDefault`/`emergencyDefault` entirely; the description precedence now matches this exact 4-tier chain, with no extra tier.
- `event.seo` overrides remain fully optional — nothing was bulk-copied into any Event's SEO block. A live, published-perspective audit confirmed no Event currently has an EN `seo.title` override, so every live Event today legitimately exercises the `documentContent` (generated) tier.

### 4. Studio SEO preview — what changed

`sanity/components/SeoObjectInput.tsx` rewritten to show, for the selected locale: the exact effective title, the exact effective description, the exact canonical URL (`https://ro-rum.dk/...`), and a manager-friendly bilingual source badge per field — `"Page-specific SEO value"`, `"Generated from event title/description"`, `"This page's approved default"`, `"Site default"`, or `"Emergency fallback"` (never `"your override"` or `"using fallback"` without the actual text, never `"(not set for this language...)"`). A muted note explains that an empty document field is not a broken state when a fallback exists. An Event document's locale selector now reads `visibleLocales` (`useFormValue`, same mechanism `EventLocaleAwareInput.tsx` already uses) and only offers locales the event is actually enabled for — Danish is never offered as a preview language for a Ukrainian-only event, and vice versa. Every other document (page/legalPage/siteSettings) still always offers EN/DA/UK. `siteSettings.defaultSeo` is fetched live via `useClient` (the same pattern `ContactDetailsOrderInput.tsx` already established) so the `siteDefault` tier is genuinely live data, not stale.

### 5. Live Sanity audit and correction

Raw-perspective read (before any write) of both `siteSettings` and `drafts.siteSettings`:

| Document | `website` | `siteUrl` (before) | `defaultSeo` |
|---|---|---|---|
| `siteSettings` (published) | `"ro-rum.dk"` (correct) | `"https://rorum.dk"` (wrong) | unset |
| `drafts.siteSettings` | `"ro-rum.dk"` (correct) | `"https://rorum.dk"` (wrong) | present but every entry (title/description/ogImage.alt, EN only) has no `value` — stray, valueless residue, same class of issue as this project's earlier `repair-*-seo-residue.ts` fixes, **not touched** (out of this task's explicitly authorized scope) |

- **Backup**: new `scripts/backup-site-settings.ts` wrote both documents to a timestamped JSON file before any write.
- **Applied**: new `scripts/repair-site-settings-domain.ts` (dry-run by default, revision-guarded, re-fetch-before-write) patched **only** `siteUrl` on both documents to `"https://ro-rum.dk"`. `website`, `defaultSeo` (including its stray residue), `companyName`, `cvr`, and every announcement field were confirmed byte-identical before/after.
- **Re-run dry-run**: both documents report "already correct, no change needed" — confirmed idempotent, zero pending.
- No Sanity Publish action was invoked; no page draft was published; no other document was touched.

### 6. Redirect / hosting status

`middleware.ts` gained a host/protocol check (runs before the existing locale-rewrite logic, same file, same request pipeline): a request with `Host: rorum.dk` (any protocol), or `Host: ro-rum.dk` with `x-forwarded-proto: http`, redirects (308) to the equivalent `https://ro-rum.dk` URL, preserving path and query string exactly. This is a genuine, tested (`tests/seo.spec.ts`) application-level redirect — **but it is not sufficient on its own** and does not configure DNS, TLS, or Netlify domain ownership:

- It only ever fires for a request that has already reached this deployment — which requires `rorum.dk` to first be added as a domain alias to this Netlify site (with its own DNS + SSL certificate provisioning), a manual, account-holder-only action this session has no access to perform or verify.
- Its own `matcher` (shared with the existing locale-rewrite logic) excludes `/studio`, `/api`, and any dotted static-asset path — so even once the domain is connected, this code-level redirect covers page routes only.
- **No `netlify.toml` was added.** Domain ownership of `rorum.dk` could not be verified from within this repository (no DNS lookup tool, no Netlify account access), and the task's own explicit instruction is to give manual configuration instead of adding a redirect for an unverified/unowned domain.

**Exact manual Netlify steps still required** (complete, authoritative fix — covers every path including assets/`/studio`/`/api`, and is unaffected by the middleware `matcher` limitation above):
1. In the Netlify dashboard for this site: **Domain management → Add a domain alias** → add `rorum.dk` (and `www.rorum.dk` if it should also redirect).
2. Confirm DNS for `rorum.dk` points at Netlify (per Netlify's own on-screen instructions once the alias is added) and wait for Netlify's automatic SSL certificate provisioning to complete.
3. Under the same site's **Domain management**, set the **primary domain** to `ro-rum.dk` (if not already) — Netlify then automatically redirects every other attached domain (here, `rorum.dk`) to the primary one, at the platform edge, with "Force HTTPS" already covering the `http://ro-rum.dk` case site-wide once enabled (also in Domain management, on by default for new sites — confirm it's checked).
4. No `netlify.toml` changes are required for this — Netlify's own primary-domain + Force HTTPS settings are the complete, dashboard-level equivalent of all 3 required redirect rules, and take effect at the edge before any request reaches this Next.js app.

### 7. Tests

New: `shared/siteIdentity.test.ts` (16 tests — canonical origin, trailing-slash normalization, `resolveProductionOrigin`'s local/preview/non-HTTPS rejection), `shared/seoResolution.test.ts` (15 tests — tier precedence, whitespace/undefined handling, canonical URL building), `sanity/components/SeoObjectInput.test.tsx` (11 tests — Home's populated values, an Event with empty SEO showing the real generated title/description, no leftover placeholder text, `visibleLocales`-gated locale selector, empty-field honesty note). Updated: `lib/seo.test.ts`, `lib/structuredData.test.ts`, `lib/siteSettings.test.ts` (now proves a stored `siteSettings.siteUrl` — even the old wrong domain — is completely ignored), `components/EventShare.test.tsx`, `tests/seo.spec.ts` (domain updated throughout; added a live proof that `SAMPLE_EVENT_ROUTE` resolves its title via the generated `documentContent` tier since no live Event has an override; added a comprehensive "no `https://rorum.dk` anywhere" gate over every static route + the sample Event route's full rendered HTML, plus sitemap.xml/robots.txt; added the 3 middleware redirect-behavior tests). `vitest.config.ts` gained `shared/**/*.test.ts` to its `include` glob (new test location). An explicit, disclosed scoping decision: no live Playwright test exercises an Event *with* an SEO override end-to-end, since none currently exists and this project's own established convention is not to commit a permanent Sanity-mutating fixture for a code path already proven at the unit/component level and live via every static route's own real override.

`npm run typecheck`: clean. `npm run lint`: 0 errors (1 new `react-hooks/set-state-in-effect` error was found and fixed — the locale selector's "fall back to the first active locale" logic was rewritten to derive during render instead of syncing via `useEffect`+`setState`). `npm run build`: succeeded. `npx vitest run`: 501/501 passed. `npx playwright test tests/seo.spec.ts tests/locale.spec.ts`: 77/77 passed. `npx playwright test tests/sanity-schema-visibility.spec.ts tests/cms-events-contract.spec.ts tests/cms-home-contract.spec.ts tests/cms-about-contract.spec.ts tests/cms-catering-contract.spec.ts tests/cms-event-decoration-contract.spec.ts tests/interactions.spec.ts`: 528 passed, 17 skipped, 6 failed — all 6 failures are the same, already-documented, pre-existing `page-catering.seo` published-vs-draft gap from Part 24 §17 (re-confirmed via `git stash` to reproduce identically on the unmodified `0f15109` baseline, unrelated to this Part). `npx playwright test tests/visual.spec.ts`: no baseline regenerated; pre-existing live-content-drift failures only (see Part 18's own `git stash`-verified precedent for this exact failure class), no new failures caused by this Part's code changes.

### 8. Manual verification checklist

1. Open `siteSettings` in Studio — confirm "Canonical site URL" now shows `https://ro-rum.dk` and is read-only (cannot be edited); confirm "Public website domain" (`ro-rum.dk`) is still editable as before.
2. Open any static page's SEO panel (e.g. Home) — confirm the preview shows the real stored title/description per locale with a "Page-specific SEO value" badge, and the canonical URL reads `https://ro-rum.dk/...`.
3. Open an Event with no `seo.title`/`.description` set — confirm the preview shows `<event title> | RORUM` and the event's own `longDescription`, each badged "Generated from event title/description", and confirm the locale dropdown only offers that event's own `visibleLocales`.
4. On the live site (EN/DA/UK, any static route and the sample Event route), view source and confirm `<link rel="canonical">`, every `hreflang` alternate, `og:url`, and every JSON-LD `url` field read `https://ro-rum.dk`.
5. Fetch `/sitemap.xml` and `/robots.txt` directly — confirm both reference `https://ro-rum.dk` only.
6. Once `rorum.dk` is connected in the Netlify dashboard (§6 above), confirm `http://rorum.dk/*` and `https://rorum.dk/*` both redirect to the matching `https://ro-rum.dk/*` URL, and `http://ro-rum.dk/*` redirects to `https://ro-rum.dk/*`.
7. Confirm the Events Listing language-filter order (fixed in `0f15109`, this Part's own baseline) is unaffected — reordering a Language option in Studio still changes its position on the live `/events` filter menu.

## Part 27 — Site-wide technical-field hygiene, Event Decoration and Host at RORUM Studio workflows (baseline `6c97183`)

### 1. Root cause

Every page except Contact/Events/Catering Menu Examples/FAQ (already corrected in earlier Parts) still showed managers raw `Key`/`Section type` fields on every section — `sectionKey`/`sectionKind` were only hidden via a growing set of per-document special cases, never site-wide. Event Decoration and Host at RORUM specifically had never been audited at all: their hero sections showed unused Photos/Video and Items controls, "What We Style"'s two intro paragraphs lived as two generic, unlabeled content rows instead of the section's own Text field, style cards/session-includes rows/package cards/steps all showed every generic `contentItem` field (itemKey, unused icon/image/link/value) instead of only what's rendered, Host's package `<select>` and Additional Services checkboxes were driven by a second, hardcoded English-only option list completely independent of the "Hosting Packages" cards (submitting the localized, renameable *label* as the stored value — renaming a package would have silently broken the `?package=` deep link), and Host's 15 gallery/session photos had zero Danish/Ukrainian alt text.

### 2. Phase 1 — site-wide `sectionKey`/`sectionKind` hiding (commit `acfc1b8`)

`sanity/schemaTypes/objects/pageSection.ts`: replaced the document-specific `CONTACT_FIXED_SECTION_KEYS`/`EVENTS_FIXED_SECTION_KEYS` constants and `isCorrectlyShapedCateringMenuSection`/`isCorrectlyShapedFaqCategorySection` helpers with one `isCorrectlyShapedSection()` predicate: `sectionKey`/`sectionKind` are hidden on **every** document once a section already has a `sectionKind` value (generalizing the exact reasoning Catering Menu Examples/FAQ categories already used). A section with no `sectionKind` yet (a stray raw section from Sanity's own generic array control) still shows both fields until filled in — never hidden-but-required. `sectionKey` keeps its existing readOnly-once-set lock. No schema field or attribute path changed — presentation only.

### 3. Phase 2 — Event Decoration

**Section identity** (confirmed by `_id`/`sectionKey`/`sectionKind`, not by displayed titles): `page-event-decoration` has 5 sections — `hero` (hero), `gallery` (gallery — media array **and** the "Suitable Decoration Formats" chips, both genuinely rendered from the same section, not two separate concerns as the task's own framing assumed), `styling` (split — "What We Style"), `steps` (steps — "3-step setup"), `inquiryForm` (form).

- **Hero**: `media`/`items` force-hidden (confirmed unused — the section's real CTA is `actions`, its real media lives in the separate `gallery` section). `actions`/`label`/`title`/`text` stay visible (all genuinely read). Discovered, disclosed, not changed: the hero's own `request` action's `href` is stored but never read — `EventDecorationPage`'s JSX hardcodes `#decoration-inquiry` directly; only the action's `label` is used. Left as-is (the shared `ctaAction` type is used correctly elsewhere; this is a page-specific dead sub-field, not a schema defect).
- **Gallery**: `ariaLabel`/`suitableFor0-7` items and the section's own `label` ("Suitable for:") were **already** correctly scoped by the existing, non-document-scoped "Catering gallery chip-group aria label"/"Catering 'suitable for' chip" roles (same `sectionKey`/`itemKey` shape as Catering's own gallery) — no new rule needed, confirmed live and by a new regression test.
- **Styling ("What We Style")**: migrated the two `intro0`/`intro1` generic rows into the section's own real `text` field (see §5). New `contentItem.ts` role "Event Decoration 'What We Style' item" (`documentIds: ["page-event-decoration"]`, itemKeyPattern `format\d*` incl. blank) shows icon/Title/Text **plus** an optional Link destination/text — not currently used by any live item, but kept genuinely visible and wired into the frontend (a conditional link renders under the card text only when both `href` and `label` are set), per the explicit instruction to keep it functional rather than a fake field. `tailoredNote`'s role was generalized (Catering's existing "tailored upon request" note role now also matches `sectionKeys: ["philosophy", "styling"]`) rather than duplicated. The existing "Catering 3-step setup row" role already covers `steps`' `step0-2` items unchanged (same shared, non-document-scoped role).
- **Suitable Decoration Formats**: confirmed this is not a separate section — it's the `gallery` section's own `items` (see above); already Icon + Title only via the shared Catering role.
- **Alt-text audit**: live raw-perspective query confirmed all 14 gallery photos + the styling section's 1 image already have complete EN/DA/UK alt (zero missing, zero malformed) on both published and draft — **no repair needed**.

### 4. Phase 2 data migration — styling.text (backup → dry-run → apply → idempotency proof)

- **Backup**: `scripts/backup-event-decoration-styling.ts` → both `page-event-decoration` and `drafts.page-event-decoration`.
- **Migration**: `scripts/migrate-event-decoration-styling-text.ts` reads `intro0.text`/`intro1.text` live (never hardcodes the source text), joins each language with a real `\n\n` paragraph separator, prints the exact computed value for review, then (only on `--apply`) sets `styling.text` to that value and unsets `intro0`/`intro1` in one revision-guarded transaction per document. Dry-run output confirmed the joined EN/DA/UK text exactly concatenates the two approved paragraphs before any write. Applied to both documents; second dry-run reports "no intro0/intro1 items found" (zero pending, idempotent).
- **Frontend**: `app/[locale]/(site)/event-decoration/page.tsx`'s `stylingIntro` now reads `stylingSection.text` first (split on `\n\n`, rendered as separate `<p>` elements — same DOM/visual spacing as before), falling back to the old itemKey-based read only if a document somehow still has `intro0`/`intro1` (a legacy-rollout safety net, not expected to ever fire post-migration — remove once confirmed dead), then the legacy `eventsPage` singleton, then the hardcoded emergency fallback.

### 5. Phase 3 — Host at RORUM

**Section identity**: `page-host-at-rorum` has 6 sections — `hero`, `gallery` (media-only, confirmed — no items/label stored), `session` (split — "Each Session Includes"), `packages` (cta — "Hosting Packages"), `steps` (steps), `inquiryForm` (form).

- **Hero**: same `media`/`items` force-hide as Event Decoration (confirmed unused — the hero's real gallery lives in the separate `gallery` section; both `apply`/`packages` actions' `href`s are likewise hardcoded in JSX, only their labels read — same disclosed, unchanged, page-specific dead sub-field as Event Decoration's hero action).
- **Session (Each Session Includes)**: new role "Host at RORUM session-includes item" (`included0-6`/`optional0-1`/`optionalLabel`, title-only, required) — the existing 4/3 visual column split is derived by array **position** (`.slice(0,4)`/`.slice(4,7)`, unchanged; the code-side icon list has no schema field), disclosed as-is rather than restructured into two separate arrays (would be a larger, higher-risk reshaping migration outside this task's proportionate scope).
- **Hosting Packages**: new role "Host at RORUM package" (`title`=name, `label`=price, `text`=one included line per language-line, `itemKey`/icon/image/href/value hidden) plus "Host at RORUM packages title-only row" for `footerCtaLabel`/`footerText`/`selectPackageCta`/`cancellationTitle`/`cancellation0-2`. New role "Host at RORUM step-list aria label" for the steps section's own `requestProcessAriaLabel` row (distinct from the shared step-row role).
- **Package selector — real defect fixed**: `PackageGrid`'s `?package=` deep-link and the booking form's `<select>` (`components/InquiryForm.tsx`) both used to be driven by **separate** lists — the cards from `packagesSection.items`, the form's `bookingPackageOptions` from a hardcoded English array whose `<option>` had no distinct `value`, so the submitted/deep-linked identifier **was** the localized display title. `components/Cards.tsx`'s `PackageItem` gained a `value: string` field (the package's own stable `itemKey`, e.g. `"package0"`); `PackageGrid` now builds `?package=${value}`, not `?package=${title}`. `host-at-rorum/page.tsx` computes `packageOptions` from the exact same `packageItems` array the cards render, and passes it to `InquiryForm`, which now renders its `<select>` from that canonical source (falling back to a small built-in list only when Sanity is unavailable). Renaming a package's title in Studio no longer changes what gets submitted or deep-linked; a package removed from the array disappears from both the cards and the selector together.
- **Additional Services — real defect fixed**: `bookingServiceOptions` (hardcoded, English-only: Breakfast/Snacks/Lunch/Coffee setup) is replaced by a canonical, localized source — new role "Host at RORUM additional service" for 4 new `inquiryForm` items (`service0-3`, title-only, stable `itemKey` value). `InquiryForm.tsx` renders its checkboxes from the caller-supplied `serviceOptions` (falling back to the same 4 English labels only when absent).
- **FAQ prompt**: confirmed already correct — both pages render the shared `<FAQInlinePrompt />` with no `question`/`label` override, so both already read the shared `formMessages.faqQuestion`/`.faqLabel` singleton via `useFormContent()`. No change needed.
- **Steps**: the existing shared "Catering 3-step setup row" role already covers `step0-2` unchanged.

### 6. Phase 3 data migrations

- **Backup**: `scripts/backup-host-at-rorum-docs.ts` → both `page-host-at-rorum` and `drafts.page-host-at-rorum`.
- **Gallery/session alt DA/UK backfill**: live audit found all 14 gallery photos + the session section's 1 image had EN-only alt (zero malformed entries — a genuine completeness gap, not residue) on both published and draft. `scripts/backfill-host-gallery-alt.ts` reads each EN value live, matches it against a translation table (accurate literal descriptions of each photo's visible content, same convention as this project's earlier Catering/Event Decoration alt backfills), and inserts only the missing DA/UK entries via one `.insert()` per language per transaction (never chained on one Patch — see the project's own established fix for that class of bug). Existing EN values and any existing valid translation are never touched. Applied: 30 entries per document (15 photos × 2 languages) × 2 documents = 60 total. Second dry-run: 0 missing (idempotent).
- **Additional Services migration**: `scripts/migrate-host-additional-services.ts` adds `service0-3` (Breakfast/Snacks/Lunch/Coffee setup) to `inquiryForm.items` on both documents, with new Danish/Ukrainian translations of this already-live, already-approved English text (Morgenmad/Snacks/Frokost/Kaffestation; Сніданок/Снеки/Обід/Кавова станція). Applied: 4 items × 2 documents = 8 inserts. Second dry-run: 0 missing (idempotent). **Translation-provenance disclosure**: unlike the alt-text backfill (translating already-multilingual captions), these 4 short, unambiguous food/service words are new Sanity content translated from already-live English UI text — judged low-risk and consistent with this project's established "accurate DA/UK translations derived from approved English meaning" convention, applied here under this task's own "preserve existing content" migration authorization.

### 7. Commit boundary — disclosed deviation

The task requested separate commits per phase (`fix(sanity): hide technical section identifiers` / `fix(sanity): simplify Event Decoration editing` / `fix(sanity): add manager-friendly Host workflows`). Phase 1 is its own commit (`acfc1b8`). Phases 2 and 3, however, share two foundational schema files edited in the same passes — `sanity/schemaTypes/objects/pageSection.ts`'s hero force-hide covers both pages in one combined condition, and `sanity/schemaTypes/objects/contentItem.ts`'s new roles for both pages were added together — splitting them would mean artificially fragmenting single logical hunks across two commits with no clean boundary. **Phases 2 and 3 are combined into one commit** covering Event Decoration and Host at RORUM together; this deviation is disclosed here and in the final report rather than hidden.

### 8. Tests

New: `tests/sanity-schema-visibility.spec.ts` gained a site-wide Phase 1 regression describe block (FAQ/Contact/Catering/Catering Menu Examples/Events/Event Decoration/Host at RORUM/Home/About all hide `sectionKey`/`sectionKind` once shaped; none hide it while unshaped), a hero media/items describe block (Event Decoration + Host hidden, Home unaffected, other sections on the same documents unaffected), an Event Decoration item-role describe block (What We Style/tailored-note/gallery-chip/steps-row roles), and a Host at RORUM item-role describe block (session-includes/package/step-aria-label roles) — 41 new tests total. Fixed 5 pre-existing tests whose assertions encoded the OLD document-scoped `sectionKey`/`sectionKind` behavior (now correctly hidden site-wide instead of only on specific documents) and one whose exact role-name string changed (`'Catering "tailored upon request" note'` → `'"Tailored upon request" note (Catering + Event Decoration)'`). New `components/InquiryForm.test.tsx` (9 tests): package/service options render from the caller-supplied canonical arrays, renaming a label preserves its stable value, a removed option disappears from the form, both fall back to built-in options when absent.

`npm run typecheck`: clean. `npm run lint`: 0 errors (one new `react-hooks/exhaustive-deps` warning was found and fixed by memoizing `resolvedPackageOptions` on the actual `packageOptions` prop). `npm run build`: succeeded. `npx vitest run`: 510/510 passed. `npx playwright test tests/sanity-schema-visibility.spec.ts`: 296/296 passed. `npx playwright test tests/interactions.spec.ts tests/locale.spec.ts tests/cms-home-contract.spec.ts tests/cms-about-contract.spec.ts tests/cms-catering-contract.spec.ts tests/cms-events-contract.spec.ts tests/cms-event-decoration-contract.spec.ts tests/seo.spec.ts`: 332 passed, 17 skipped, 7 failed on first run — 6 are the same already-documented pre-existing `page-catering.seo` published-vs-draft gap; the 7th (`cms-events-contract.spec.ts`'s pagination-click timeout on an unrelated Events-listing cross-page-consistency test) was verified via `git stash` to pass cleanly on the unmodified `6c97183` baseline AND to pass again when re-run in isolation on this branch — confirmed transient flakiness under parallel test load, not a real regression, not silently dismissed. No dedicated Host at RORUM CMS-contract spec file exists in this project yet (only Home/About/Catering/Events/Event Decoration have one) — out of scope to create from scratch here; the package-selector/Additional-Services architecture fix is instead covered by the new `InquiryForm.test.tsx` component tests plus the full interactions/locale/build sweep above.

### 9. Manual Studio checklist

See the task's own checklist, reproduced verbatim in the final chat report.

## Part 28 — Community Membership Studio workflow (baseline `9888a74`)

### 1. Root cause

`page-community-membership` had never been audited: the hero's two intro paragraphs lived as two generic, unlabeled `intro0`/`intro1` rows instead of the section's own Text field; every donation-section Bank Detail row unconditionally showed a Copy button regardless of whether copying that value made sense — `page.tsx`'s live-data mapping hardcoded `copyable: true` for all 9 rows, silently overriding the component's own more selective `defaultBankFields` fallback intent (only Account No./IBAN); the hero's external WECODA-website link was hardcoded three times in JSX instead of reading the already-correctly-stored `getAction(heroSection, "external")?.href`; the hero's "Become a Member" action had an EN-only label while the semantically identical action on the Application section already had approved Danish/Ukrainian translations; 8 gallery photos/videos plus the donation QR image had EN-only alt text; and every `contentItem`/section-level generic field on this page's rows was fully visible in Studio with no scoped roles at all.

**Brand name, per the task's own explicit provenance requirement**: confirmed as **WECODA** (Women Entrepreneurs Commerce & Development Association) from 5+ independent live sources — the page's own hero copy, the CVR-registered legal beneficiary name on the live Bank Details (`bank0`), the stored `wecoda.org` external link, the gallery photos' own alt text, and the existing static QR/logo image filenames (`wecoda-logo.jpg`, `wecoda-donation-qr.jpg`). No alternate spelling ("Vcode"/"Wecoda" mis-hearing) appears anywhere in stored data or existing project copy — nothing was renamed.

### 2. Site-wide technical-field hygiene reuse

No new mechanism needed — Phase 1's `isCorrectlyShapedSection()` (Part 27) already hides `sectionKey`/`sectionKind` on this document once each section has a `sectionKind`, confirmed by the existing site-wide regression tests (no page-community-membership-specific carve-out required).

### 3. Studio field visibility (`sanity/schemaTypes/objects/pageSection.ts`, `contentItem.ts`)

**Section identity**: `page-community-membership` has 6 sections — `hero`, `donation`, `intro` ("Connecting Women"), `benefits`, `application`, `gallery`.

- **Hero**: `media` force-hidden (the hero photo is a static logo image, not a manager-uploaded media item). `text` stays **visible** — unlike Event Decoration/Host's own hero (Part 27), this section's `text` is the real, now-migrated two-paragraph intro field.
- **Intro ("Connecting Women")**: `text`/`media`/`actions` force-hidden — the real paragraph content lives in the section's `items[].text` (`column0`/`column1`), and its visible action buttons are cross-wired from the hero section's own actions (a disclosed, unchanged, pre-existing reuse — not a hidden dead field on this section).
- 7 new `contentItem.ts` roles, all `documentIds: ["page-community-membership"]`-scoped: **annual price strip** (hero's `priceStripText`, Title only, required — kept as one full sentence, not split into a separate amount/currency shape, per the "preserve the compatible stored shape" instruction); **donation short-label rows** (`scanText`/`scanSubtext`/`orText`/`bankTransferText`/`bankDetailsTitle`, Title only — currently unpopulated live, correctly falling back to the page's own hardcoded default text, confirmed genuinely read, not dead residue); **donation closing note** (`supportText`, Text only); **bank detail** (`bank\d*`, Title/Row value/Copy button enabled, Title+Value required — one canonical role for all 9 rows); **intro column** (`column\d*`, Text only, required); **benefit** (`benefit\d*`, Title/Text/Image, Title+Text required — the Lucide icon field is deliberately hidden, see §5); **application step** (`step\d*`, Title/Text only).

### 4. Bank Details — real defect fixed

`app/[locale]/(site)/community-membership/page.tsx`'s live-data mapping hardcoded `copyable: true` for every bank row regardless of the field's own value or intent. Fixed in two parts:
- **Schema**: new `copyEnabled` boolean field on `contentItem` (`initialValue: false`), hidden by role everywhere except the "Community Membership bank detail" role.
- **Data**: `scripts/migrate-community-membership-bank-copy.ts` sets `copyEnabled: true` on exactly `bank4` ("Account No.") and `bank6` ("IBAN") — confirmed live to match `WecodaDonationSection.tsx`'s own `defaultBankFields` fallback intent — leaving the other 7 rows at the schema default `false`. Applied to both documents (2 updates each); second dry-run: 0 pending (idempotent).
- **Frontend**: `bankFields` mapping now reads `copyable: f.copyEnabled ?? false` instead of hardcoding `true`, and filters out any row with an empty `value` before rendering (`no empty Bank Detail row renders; no Copy button renders for an empty value`, per the task's own stated correct behavior — a defensive fix; all 9 live rows currently have real values, so this doesn't change today's render, only protects a future empty row). Verified against the built HTML: exactly 2 `Copy` buttons render (Account No., IBAN), all 9 labels present with real values.

### 5. Membership Benefits — image authority (already satisfied)

Live tracing confirmed `page.tsx`'s `benefits` mapping already prioritizes `urlForImage(b.image)` over any hardcoded icon, with a bundled fallback PNG (never a Lucide icon) as the deepest fallback — the frontend already treats the Sanity image as authoritative. The only remaining defect was Studio surface: the unused Lucide `icon` picker field was fully visible on every benefit row. Fixed by the new "Community Membership benefit" role (§3), which shows Title/Text/Image and hides `icon`. No frontend or data change was needed here.

### 6. Hero intro paragraph migration (backup → dry-run → apply → idempotency proof)

Live audit confirmed `hero.items[intro0]`/`[intro1]` hold real, non-empty, actively-rendered paragraph content (not unused residue, contrary to the task's own tentative "verify empty then remove" framing) — correctly redirected to this project's proven "migrate into the section's own `text` field" pattern (Part 27 §4), reused a third time.
- **Backup**: `scripts/backup-community-membership-docs.ts` → both `page-community-membership` and `drafts.page-community-membership`.
- **Migration**: `scripts/migrate-community-membership-hero-text.ts` (byte-for-byte adaptation of `migrate-event-decoration-styling-text.ts`) reads `intro0.text`/`intro1.text` live, joins each language with `\n\n`, sets `hero.text`, unsets `intro0`/`intro1` — one revision-guarded transaction per document. Dry-run output confirmed the joined EN/DA/UK text exactly concatenates the two approved paragraphs (EN: "WECODA: Women Entrepreneurs Commerce & Development Association." + "Connect with women entrepreneurs…"). Applied to both documents; second dry-run reports "no intro0/intro1 items found" (idempotent).
- **Frontend**: `heroIntro` now reads `heroSection.text` first (split on `\n\n`, rendered as the same two `<p>` elements with unchanged visual spacing), falling back to the old itemKey-based read, then the legacy singleton, then the hardcoded fallback. Confirmed in the built HTML: two separate `<p class="wecoda-hero-intro">` paragraphs render, byte-identical to the approved copy.

### 7. External WECODA link — real defect fixed

The hero's external-site link (`https://wecoda.org`) was hardcoded in JSX three times (hero actions, intro section, membership panel) despite `getAction(heroSection, "external")?.href` already correctly storing that exact URL. Fixed: new `externalSiteHref = getAction(heroSection, "external")?.href || fallbackWecodaExternalUrl`, all 3 JSX occurrences now read `data.externalSiteHref`. Verified in the built HTML: all 3 anchors resolve to `https://wecoda.org` via the computed value, not the literal.

### 8. Hero "apply" action DA/UK label — real defect fixed

`scripts/migrate-community-membership-hero-apply-label.ts` copies the already-approved Danish/Ukrainian labels ("Bliv medlem"/"Стати учасницею") from the Application section's own "apply" action (identical href, identical English label — the same semantic action duplicated across two sections) onto the hero's own "apply" action, which was EN-only. No new translation invented — the value written is byte-identical to the value already approved and live elsewhere. Applied to both documents; second dry-run: already complete (idempotent). Confirmed in the built HTML: `/da/community-membership` and `/uk/community-membership` both render the correct localized label on the hero's primary CTA.

### 9. Alt-text backfill — 8 gallery items + 1 QR image (DA/UK)

Live audit found all 8 gallery photos/videos and the donation QR image had EN-only alt (zero malformed entries). `scripts/backfill-community-membership-alt.ts` reads each EN value live, matches it against a translation table (accurate literal descriptions, same convention as the Catering/Event Decoration/Host backfills), inserts only the missing DA/UK entries. Applied: 18 entries per document (9 items × 2 languages) × 2 documents = 36 total. Second dry-run: 0 missing (idempotent).

### 10. Gallery — resolver-duplication decision (disclosed)

Evaluated reusing `lib/sanityGallery.ts`'s `resolveCanonicalGalleryItems`/`resolveGalleryItems` (the shared "missing vs. intentionally empty" gallery resolver used by Catering/Event Decoration/Host at RORUM) directly for this page's gallery. Rejected direct reuse: this page's mosaic layout needs a `featured` (2×2 hero tile) flag meaningless to every other gallery consumer, and its static emergency fallback mixes photos and videos while the shared resolver's own fallback tier is images-only (`kind: "image"` hardcoded) — forcing the fit would either leak a page-only concern into the shared type or silently drop the video fallback items. Instead, the exact same **policy** (missing section → legacy/static fallback; section present but `media` empty → render nothing, never resurrect legacy/static) is mirrored by hand in `page.tsx`'s own `resolveMembershipMedia()` helper, which also now mirrors `resolveOne`'s photo/video precedence (uploaded file before external URL, rejecting a non-direct-file URL) — fixing a latent second defect where the page's own duplicate resolver never checked for an uploaded video file at all, only `videoUrl`. This is disclosed as a deliberate, reasoned divergence from full reuse, not an oversight.

### 11. Tests

New: `tests/sanity-schema-visibility.spec.ts` gained a "Community Membership hero/intro force-hidden fields" describe block (5 tests: hero media hidden/text visible, intro text/media/actions all hidden, regression on unrelated documents, regression on the page's own other sections) and a "Community Membership reserved item roles" describe block (7 tests, one per new role, plus a cross-document non-activation regression) — 12 new tests total, all also covered by the existing generic `ITEM_ROLE_RULES` self-check loop (extended with new candidate itemKeys and a `copyEnabled` field check). New `components/WecodaDonationSection.test.tsx` (8 tests, mocked Clipboard API per the task's explicit requirement): Copy renders only for `copyable: true` rows (never for an empty-value or non-copyable row), the real `defaultBankFields` show exactly 2 Copy buttons, no Copy button renders when every row is non-copyable, clicking Copy writes the row's exact value via the mocked `navigator.clipboard.writeText`, success feedback shows and auto-reverts after the timeout, copying one row never marks a different row copied, the button is keyboard-activatable (Tab + Enter), and success feedback still shows when the Clipboard API rejects/is unavailable (manual copy remains possible).

`npm run typecheck`: clean. `npm run lint`: 0 new errors (one pre-existing `<img>` LCP warning on this page, unchanged, matching the same warning already present on every other gallery-rendering page). `npm run build`: succeeded, all 3 locales prerendered. `npx vitest run`: 518/518 passed (38 files). `npx playwright test tests/sanity-schema-visibility.spec.ts`: 316/316 passed. `npx playwright test tests/interactions.spec.ts tests/breakpoints.spec.ts tests/locale.spec.ts`: 71 passed, 1 skipped (unchanged from baseline). `npx playwright test tests/cms-about-contract.spec.ts tests/cms-catering-contract.spec.ts tests/cms-event-decoration-contract.spec.ts tests/cms-events-contract.spec.ts tests/cms-home-contract.spec.ts`: 230 passed, 16 skipped, 6 failed — all 6 are the same already-documented pre-existing `page-catering.seo` published-vs-draft gap (Part 22/27), unrelated to this work.

`npx playwright test tests/visual.spec.ts`: 29 failed on first run, spanning many pages this session never touched (`/host-at-rorum`, `/catering`, `/contact`, `/privacy-policy`, `/cookie-policy`, `/volunteer`, `/work-with-us`, `/events/copenhagen-makers-dinner`) plus `/community-membership`. Verified via `git stash -u` (full A/B baseline comparison, not assumed): rebuilding on the unmodified pre-session code reproduces the **identical** failures on every one of those unrelated pages — confirmed pre-existing baseline drift, not caused by this session's work. `/community-membership`'s own 4 failures were investigated separately (stash restored, rebuilt on the real changes) and are the expected, disclosed consequence of the Bank Details Copy-button fix (§4: 7 fewer Copy buttons now render, reflowing row heights by a few px) — not a defect. Per the task's explicit "no visual-baseline regeneration" instruction, **no baseline was regenerated**; both the pre-existing unrelated failures and the Community-Membership-specific expected diff are reported here for review rather than silently resolved.
