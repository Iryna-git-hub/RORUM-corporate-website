      ...
      <section className="book-space-hero">
        <Container>
          <div className="book-space-hero-copy">
            ...
            <p>Use RORUM for private meetings, team sessions, planning days, creative workshops and intimate gatherings.</p>
          </div>
          <HorizontalGallery images={galleryImages}/>
        </Container>
      </section>
      <Section><Container><SectionHeader label="Use cases" title="Flexible without feeling blank." level={3}/><div className="grid-3">{["Team meeting", "Creative workshop", "Private gathering"].map((item) => <EditorialCard key={item} title={item} text="A refined environment with modular tables, warm light and optional hospitality."/>)}</div></Container></Section>
      <Section tight><Container><SectionHeader label="Formats" title="Morning, afternoon or full day." level={3}/><PackageGrid items={packages.booking}/></Container></Section>
      <Section><Container><div className="split"><Card className="card-pad"><SectionLabel>Included and policy</SectionLabel><ul className="clean-list"><li>Tables, seating and simple room reset</li><li>Coffee, tea and food add-ons by request</li><li>Indicative cancellation policy for MVP: confirmed during quote</li></ul></Card><InquiryForm type="booking" title="Private meeting inquiry"/></div></Container></Section>
    </>);
}
